...
def calculate_entropy(cov: np.ndarray):
...
def greedy_batch_selection(
    K_train_train: np.ndarray, K_train_candidates: np.ndarray, K_candidates_candidates: np.ndarray, batch_size: int
):
    ...
    for _ in range(batch_size):
        ...
        for idx in range(len(remaining_candidates)):
            ...
            combined_cov = update_covariance_matrix(
                K_train_train, K_train_candidates[:, [idx]], candidate_cov
            )
        ...
        best_candidate_cov = K_candidates_candidates[
            np.ix_([best_candidate_idx], [best_candidate_idx])
        ]
        K_train_candidates_best = K_train_candidates[:, [best_candidate_idx]]

        K_train_train = update_covariance_matrix(
            K_train_train, K_train_candidates_best, best_candidate_cov
        )

        K_train_candidates = np.delete(K_train_candidates, best_candidate_idx, axis=1)
        new_row = K_candidates_candidates[
            best_candidate_idx, remaining_candidates_idx
        ].reshape(1, -1)
        K_train_candidates = np.vstack([K_train_candidates, new_row])

        K_candidates_candidates = np.delete(
            K_candidates_candidates, best_candidate_idx, axis=0
        )
        K_candidates_candidates = np.delete(
            K_candidates_candidates, best_candidate_idx, axis=1
        )

    return selected_indices


def update_covariance_matrix(K_train_train: np.ndarray, K_train_candidate: np.ndarray, candidate_cov: np.ndarray):
...
## Acquisition functions from Batched Bayesian optimization with correlated candidate uncertainties arXiv:2410.06333 [cs.LG]
...
def acquire_gPO(
    ...
    **kwargs
    ...
    try:
        ...
        while count < 10 and not sampled:
            ...
            try:
                ...
                p_yx = multivariate_normal(
                    mean=mean, cov=cov, allow_singular=True, seed=seed
                )
                samples = p_yx.rvs(size=Ns, random_state=seed)
                sampled = True
            except:
    ...
    probs = (
        np.bincount(top_samples, minlength=len(mean)) / Ns
    ) 

    return np.argsort(probs)[::-1][:batch_size], np.array(probs)[np.argsort(probs)[::-1][:batch_size]]


def acquire_ts(
    mean, cov, c: int = 1, batch_size: int = 100, seed: int = None, **kwargs
):
