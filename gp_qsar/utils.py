...
def scaffold_split(smiles, y, random_state, test_size, cutoff):
    ...
    for cluster in remaining_clusters:
        ...
        clust_train, clust_test = train_test_split(idx_list, test_size=updated_split, random_state=random_state)
...
def splitter(smiles, y, method="random", random_state=42, test_size=0.2, n_bins=5, cutoff=0.3):
    """
    Split a set of smiles and associated target variables by various methods.
    """
    if method.lower() == "random":
        return train_test_split(smiles, y, test_size=test_size, random_state=random_state)
    ...
    raise ValueError(f"Unknown method '{method}'. Supported methods are: 'random', 'stratified', 'scaffold'")
